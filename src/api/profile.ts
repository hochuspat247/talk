...
export const getProfile = async (userId: number): Promise<User> => {
  try {
    const response = await client.get(`/api/profile/${userId}`); // Исправляем синтаксис
...
export const updateProfile = async (userId: number, updatedData: Partial<User>): Promise<User> => {
  try {
    const response = await client.patch(`/api/profile/${userId}`, updatedData); // Исправляем синтаксис
    return response.data;
  } catch (error: any) {
